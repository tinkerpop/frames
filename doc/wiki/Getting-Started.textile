...
There are a few things that are worth exemplifying. First, while the property of the project vertex may have the key @lang@, the method to get and set the property value can be anything. The @@Property@ annotation makes it clear what the explicit property key is. Second, notice that there is no explicit @created_by@ edge in the example graph. We can infer this relation by using inverting a @created@ edge. In other words, the people who create a project are the people that the project was created by. Third, note that there is a difference between a *Relation* and an *Adjacency*. A relation exists between two vertices. An adjacency exists between a vertex and edge. An adjacency is a @EdgeFrame@ and thus, a wrapper to an edge.
